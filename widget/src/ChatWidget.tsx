...
import { ProductCard, WidgetAPI, WidgetConfig } from "./api";
import { initAutoAddWatcher, requestAddToCart } from "./cartBridge";
...
const BotMascotSvg = () => (
  ...
  >
    ...
    <path
      d="M13.5 19C14.2 19.8 15 20.2 16 20.2C17 20.2 17.8 19.8 18.5 19"
      stroke="#ffffff"
      strokeWidth="1.5"
      strokeLinecap="round"
    />
    <path
      d="M5 14C5 13 5.8 12.2 6.8 12.2V17.8C5.8 17.8 5 17 5 16V14Z"
      fill="#0f172a"
    />
    <path
      d="M27 14C27 13 26.2 12.2 25.2 12.2V17.8C26.2 17.8 27 17 27 16V14Z"
      fill="#0f172a"
    />
  </svg>
);
...
export function ChatWidget({ api }: ChatWidgetProps) {
  ...
  const [windowOffset, setWindowOffset] = useState<{ x: number; y: number }>({
    x: 0,
    y: 0,
  });
  ...
  useEffect(() => {
    ...
    const calculateSnap = (
      ...
      const elemWidth =
        target === "window" ? (windowWidth <= 640 ? windowWidth : 390) : 60;
      const elemHeight =
        target === "window" ? (windowHeight <= 640 ? windowHeight : 580) : 60;
      ...
      const snapLeft =
        absLeft < (windowWidth - elemWidth) / 2
          ? 24
          : windowWidth - 24 - elemWidth;
      const snapTop =
        absTop < (windowHeight - elemHeight) / 2
          ? 24
      ...
      if (dragStartRef.current && didDragRef.current) {
        ...
        if (target === "window") {
          ...
          setLauncherOffset((prev) =>
            calculateSnap("launcher", prev.x, prev.y),
          );
        }
      }
    ...
    const onDone = (res: any) => {
      ...
      if (res.cartAction) {
        ...
        const targetProd = (res.products || []).find(
          (p: any) =>
            p.id === actionProdId ||
            (p as any).externalId === actionProdId ||
    ...
    >
          ...
          >
            ...
            >
              <input
                ...
                maxLength={250}
                onInput={(e: any) =>
                  setInputValue((e.target.value || "").slice(0, 250))
                }
                style={{
                  flex: 1,
                  padding: "9px 0",
                ...
                {(modalProduct.options || []).map((opt) => (
                  <div key={opt.name} style={{ marginBottom: "12px" }}>
                    ...
                    >
                      {opt.values.map((val) => {
                        const isSelected =
                          selectedOptionsState[opt.name] === val;
                        return (
                          <button
                            key={val}
                            type="button"
                  ...
                  onClick={() => {
                    const selectedVariant = modalProduct.variants?.find((v) => {
                      ...
                      return Object.entries(selectedOptionsState).every(
                        ([k, val]) =>
                          String(v.options?.[k]).toLowerCase() ===
      ...
      <button
        ...
        style={{
          ...
          userSelect: "none",
          transform:
            launcherOffset.x !== 0 || launcherOffset.y !== 0
