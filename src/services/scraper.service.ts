...
export async function scrapeWebsite(targetUrl: string, merchantId: string): Promise<ScrapeResult> {
    ...
    try {
      ...
      for (const prod of productsFound) {
        ...
        try {
          ...
          await prisma.$executeRawUnsafe(`UPDATE "Product" SET embedding = $1::vector WHERE id = $2`, vectorStr, savedProduct.id).catch(() => {});
  ...
  logger.info(`Scraper: Deep crawl complete. Visited ${visitedUrls.size} pages. Indexed ${totalIndexedProducts} products and ${totalKnowledgeChunks} knowledge chunks.`);
