...
export async function getAdminOverview(
  ...
  try {
    const [
      ...
      (prisma as any).contactInquiry
        ? (prisma as any).contactInquiry
            .count({ where: { status: "NEW" } })
            .catch(() => 0)
        : 0,
      (prisma as any).bugReport
        ? (prisma as any).bugReport
    ...
    const estimatedMrr = starterTiers * 2 + proTiers * 5;
...
export async function getAdminSubscriptions(
  ...
  try {
    const [freeCount, starterCount, proCount, enterpriseCount, paidUsers] =
      await Promise.all([
        prisma.user.count({ where: { planTier: "FREE" } }),
        prisma.user.count({ where: { planTier: "STARTER" } }),
        prisma.user.count({ where: { planTier: "PRO" } }),
    ...
    const estimatedMrr = starterCount * 2 + proCount * 5;
