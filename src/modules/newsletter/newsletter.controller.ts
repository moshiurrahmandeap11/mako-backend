...
export async function subscribeNewsletter(req: Request, res: Response): Promise<void> {
  try {
    ...
    logger.info(`[Newsletter] New subscriber: ${cleanEmail} (Source: ${subscriber.source || 'website'})`);
    ...
    sendNewsletterWelcomeEmail({
      ...
      logger.error(`[Newsletter] Failed to dispatch welcome email to ${cleanEmail}:`, emailErr);
    });

    res.status(200).json({
      success: true,
      message: 'Thank you for subscribing to Labto AI updates! A welcome confirmation email has been sent.',
      subscriber: {
